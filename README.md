...
- Firefox メニュー（アドオン）から iSPEED 起動を促し、同意後に **きょうの新聞** を開く
- 設定画面で自動化のオン／オフを切り替え
...
1. Firefox のメニュー → **アドオン** から **T-Kong** をタップ  
   （項目名は「日経テレコン(きょうの新聞)」と出ることがあります）
2. iSPEED が開いたら **ログイン（認証）**
3. 右下メニュー → マーケット → **日経テレコン**
4. 許諾画面で同意（自動同意は設定でONにした場合のみ）
5. Firefox に戻ると、保存フラグに従って **きょうの新聞** を開きます

すでにテレコン（`t21.nikkei.co.jp`）を開いているときは、その場で「きょうの新聞」へ進みます。認証の代行はしません。
